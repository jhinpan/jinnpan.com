...
#   AFF: split = inherited SGLANG_SET_CPU_AFFINITY=1 (rank r gets physical cores r*32..r*32+31,
#        so TP0/TP1 land on NUMA node 0 while HIP 4-7 sit on node 1); node1 = SGLang affinity off
#        and the whole server pinned to node 1; none = SGLang affinity off, no pinning.
set -uo pipefail
C=$CAMPAIGN
E=$E2E_CAMPAIGN
PY=$CAMPAIGN_CACHE/env/bin/python
NODE1_CPUS=64-127,192-255
label=${1:?label} mode=${2:?mode} aff=${3:?aff}
port=30251
out=$C/results/$label
...
case $aff in
  split) ;;
  node1) export SGLANG_SET_CPU_AFFINITY=0; prefix=(taskset -c "$NODE1_CPUS") ;;
  none) export SGLANG_SET_CPU_AFFINITY=0 ;;
  *) echo "bad aff $aff" >&2; exit 2 ;;
esac
utc() { date -u +%FT%T.%3NZ; }
t_start=$(utc)
cat > "$out/manifest.json" <<EOF
{"label": "$label", "mode": "$mode", "affinity_mode": "$aff", "sglang_set_cpu_affinity": "${SGLANG_SET_CPU_AFFINITY:-unset}", "node1_cpus": "$NODE1_CPUS",
