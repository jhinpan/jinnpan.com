#!/usr/bin/env bash
# The e2e824 Low-Latency cell (launch_ll.sh) with BBuf's simulated acceptance:
# static verify, target accept length 5.5, match-expected. env.sh unsets the
# simulation variables, so they are exported after sourcing it.
set -euo pipefail
