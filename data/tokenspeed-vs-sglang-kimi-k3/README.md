...
**Headline:** TokenSpeed's Gluon kernels are faster on every matched kernel group
(MLA decode 2.77x, MoE routing 6.51x, total device compute 1.21x) and it issues 30%
fewer launches with the same 187 collectives per step — yet it is 1.14x slower end to
end at batch 1 and 3.7x slower at concurrency 8. The deficit is between the kernels,
not inside them.
...
- *Collectives excluded* because in eager decode the all-reduce payload is a few KB and
  the kernel busy-waits, so its duration measures rank skew, not work (67.6 ms/step of
  pure waiting against a 17 ms compute budget). Dispatch **counts** are still compared.
...
| Collective dispatches / decode step | 187 | 187 |
| End-to-end step, graph ON | 19.39 ms | 22.19 ms |

Fewer launches rules out launch overhead; identical collective counts rule out
communication volume. The remaining cost is host-side, between the kernels.
...
```bash
# build (isolated venv — TokenSpeed pins torch 2.11.0 and cannot share SGLang's env)
bash scripts/ts-build.sh && bash scripts/ts-build-step6.sh
...
# end-to-end sweep
MAXSEQS=128 WORKLOADS="1024:1024:1:4:1 1024:1024:8:16:2 1024:1024:32:64:4" \
