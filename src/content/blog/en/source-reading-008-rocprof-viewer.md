...
Seven hand-coded SVG plates and **eleven real screenshots of the viewer's own panels** (ISA, Global / Compute Unit / Utilization, Hotspot, Counters, Wave States, Occupancy, Dispatches, Summary, side panel) — so every "how to read it" sits next to the actual UI. The plates cover the capture-to-view pipeline, what the mask/select parameters carve out of the silicon, the `se_sm_sl_wv` filename decoded into a hardware coordinate, the annotated RCV window, the Utilization lanes, and **a real measured wave** from this MI350X (`topk_gating_softmax` dispatch 37 — 8480 cycles, 1162 instructions, 46 waitcnt events). Plus the full `rocprofv3` mode taxonomy (trace / PMC / PC-sampling / ATT and the five output formats), a panel-by-panel guide, a five-click reading loop, and the reefs that make ATT readings quietly wrong.

It closes with a case study that is also a confession: **why my own FlyDSL PR (#593, a manual `source_loc` pin) was closed in favor of #586's `WrapLocations` pass.** For `flash_attn_func`, 91.9% of ISA instructions collapsed onto the `@flyc.kernel` decorator line; the fix that worked pushed location attribution down into the compiler's pass pipeline, not up into the API. The viewer's source column is only as honest as the IR feeding it.
